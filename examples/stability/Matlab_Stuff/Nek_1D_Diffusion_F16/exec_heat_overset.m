...
dlmwrite('system_matrix_advdif',M)

% For MR: show timesteps on grids:
cfl = 13.82;
dt_L = cfl*min(h_L)*min(h_L);
dt_R = cfl*min(h_R)*min(h_R);
disp('dt_L=');disp(dt_L);
disp('dt_R=');disp(dt_R);
...
if strcmp(integrator,'MRAB') == 1
    SR = 6;
    dt = SR*cfl*min(h_L,h_R)*min(h_L,h_R);
else
